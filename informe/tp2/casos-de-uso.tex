...
Los pasos 1, 4, 6 y 9 se corresponden con el objetivo \textbf{lograr [ingresar datos de identificación y de domicilio] (1.1.1.1.1.1.2.1.1)}, y con el fenómeno \textbf{\ref{itm:f-cliente-datos-personal-sistema}: el cliente presenta datos de identificación y de domicilio}.

Los pasos 2 y 3 se corresponden con el objetivo \textbf{lograr [validar usuario y contraseña] (1.1.1.1.1.1.2.2.4)}.

El paso 5 responde al objetivo \textbf{lograr [evaluar estado financiero] (1.1.1.1.1.1.2.2.2)}, y se corresponde con el fenómeno \textbf{\ref{itm:f-sistema-evalua-financiera}: sistema evalúa estado financiero a través de financiera}.

El paso 7 responde al objetivo \textbf{lograr [validar domicilio] (1.1.1.1.1.1.2.2.1)}, y al fenómeno \textbf{\ref{itm:f-sistema-valida-correoargentino}: el sistema valida domicilio con el correo argentino}

El paso 8 responde al objetivo \textbf{lograr [presentar datos de pago, si quiere hacer pagos online] (1.1.1.1.1.1.2.1.2)} y al evento \textbf{\ref{itm:f-cliente-datos-pago-sistema}: cliente presenta datos de pago al sistema}.

Los pasos 10 y 11 responden al objetivo \textbf{lograr [validar mail] (1.1.1.1.1.1.2.2.5)} y al evento \textbf{\ref{itm:f-sistema-confirmacion-correoelectronico}: el sistema le indica al correo electrónico que envíe mail de confirmación al cliente.}
...
Este caso de uso se corresponde con el objetivo \textbf{lograr [validar mail] (1.1.1.1.1.1.2.2.5)}, y con el evento al evento \textbf{\ref{itm:f-sistema-confirmacion-correoelectronico}: el sistema le indica al correo electrónico que envíe mail de confirmación al cliente}.
...
Este caso de uso responde al objetivo \textbf{lograr [evaluar estado financiero] (1.1.1.1.1.1.2.2.2)}, y con el fenómeno \textbf{\ref{itm:f-sistema-evalua-financiera}: sistema evalúa estado financiero a través de financiera}.
...
Este caso de uso se corresponde con el fenómeno \textbf{\ref{itm:correoelectronico-mail-cliente}: el sistema de correo electrónico envía mail al cliente}. 
...
Este caso de uso se corresponde con el evento \textbf{\ref{itm:f-cliente-autentica-sistema}) el cliente ingresa credenciales en el sistema}, y con el objetivo \textbf{lograr [cliente se autentica de forma segura] (1.1.1.1.1.2)}, y con sus subobjetivos.
...
Los pasos 1, 2 y 4 se corresponden con el objetivo \textbf{lograr [mostrar stock disponible] (1.1.1.1.2.1)} y con el fenómeno \textbf{\ref{itm:f-sistema-muestra-stock-cliente}: el sistema muestra stock disponible al cliente}.

Los pasos 3 y 4 se corresponden con el objetivo \textbf{lograr [mostrar recomendaciones] (1.1.1.1.2.2)} y con el fenómeno \textbf{\ref{itm:f-sistema-muestra-recomendaciones-cliente}: el sistema muestra recomendaciones al cliente.}
...
Este caso de uso se corresponde con el objetivo \textbf{lograr [seleccionar mercadería] (1.1.1.1.2.3)} y con el fenómeno \textbf{\ref{itm:f-cliente-elige-mercaderia-sistema}: el cliente selecciona mercadería en el sistema}. El paso 2 se corresponde con el objetivo \textbf{lograr [mostrar stock disponible] (1.1.1.1.2.1)}.
...
Este caso de uso responde al objetivo \textbf{lograr [confirmar carrito] (1.1.1.1.2.4)}, y al fenómeno \textbf{\ref{itm:f-cliente-confirma-carrito-sistema}: el cliente confirma el carrito al sistema}.

Los pasos 1 y 1.1, se corresponden con el objetivo \textbf{lograr [si al confirmar carrito se acabó el stock de algún producto, se muestra mensaje de error y se quita producto del carrito, y se vuelve al estado previo a la confirmación] (1.1.1.1.2.5)} y con los fenómenos \textbf{\ref{itm:f-sistema-confirma-stock-cliente}: sistema ratifica al cliente stock disponible} y \textbf{\ref{itm:f-sistema-error-stock-cliente}: sistema muestra mensaje de error de producto no disponible al cliente}.

El paso 2 se corresponde con el fenómeno \textbf{\ref{itm:f-apilogistica-envia-costo-sistema}: sistema obtiene el costo del envío de la API de Logística} y con el objetivo \textbf{lograr [calcular costo de envío] (1.1.1.1.4.4)}.
...
El paso 4 se corresponde con los fenómenos \textbf{\ref{itm:f-apilogistica-fechas-libres-sistema}, \ref{itm:f-sistema-fechas-libres-cliente} y \ref{itm:f-cliente-fecha-entrega-sistema}}, y con el objetivo \textbf{lograr [acordar fecha de entrega] (1.1.1.1.3)} y sus subobjetivos.

El paso 5 se corresponde con el evento \textbf{\ref{itm:f-sistema-muestra-metodos-pago-cliente}: sistema presenta los métodos de pago disponibles al cliente} y con los objetivos \textbf{lograr [evaluar si permitir contraentrega] (1.1.1.1.4.1)} y \textbf{lograr [informar métodos de pago disponibles] (1.1.1.1.4.2)}.
...
El paso 1 se corresponde con el evento \textbf{\ref{itm:f-apilogistica-fechas-libres-sistema}: sistema pregunta próximas fechas libres a la API de logística}
El paso 2 se corresponde con el evento \textbf{\ref{itm:f-sistema-fechas-libres-cliente}: el sistema presenta las posibles fechas de entrega al cliente}
El paso 3 se corresponde con el evento \textbf{\ref{itm:f-cliente-fecha-entrega-sistema}: el cliente indica la fecha de entrega deseada al sistema}
...
Este caso de uso responde al objetivo \textbf{lograr [validar domicilio] (1.1.1.1.1.1.2.2.1)} y al fenómeno \textbf{\ref{itm:f-sistema-valida-correoargentino}: el sistema valida domicilio con el correo argentino}.
